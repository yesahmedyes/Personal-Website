  ...
  return (
    <Section title="Variational Autoencoders">
      ...
      <div>
        ...
        this distribution using an <MyLink href="em-algorithm">Expectation Maximization Algorithm</MyLink>.
      </div>
      <div>
      ...
      <div>
        From this, we can find an approximate value for <InlineMath math="p(z | x; \theta)" /> by choosing a <InlineMath math="Q" /> from our family of distributions <InlineMath math="\mathcal{Q}" />:
      </div>
      <div>
        <BlockMath math="p(z | x; \theta) \approx \argmax_{Q \in \mathcal{Q}} \left( \max_{\theta} \text{ELBO}(x; Q, \theta) \right)" />
      </div>
      <div>
        We make a mean field assumption that assumes that <InlineMath math="Q(z)" /> gives a distribution with independent coordinates which means that we can decompose <InlineMath math="Q(z)" /> into{" "}
        <InlineMath math="Q^1(z_1) \ldots Q^k(z_k)" />.
      </div>
      <div>
      ...
      <div>
        Ofcourse, since we still have an expectancy, we will have to take multiple samples of <InlineMath math="z^{(i)}" /> in case of <InlineMath math="\theta" />, and <InlineMath math="\xi^{(i)}" />{" "}
        in case of <InlineMath math="\psi" /> and <InlineMath math="\phi" /> and then take an average over the derivatives.
      </div>
