...
export default function OtherInterpretations() {
  return (
    <Section heading="Other Interpretations">
      ...
      <div>
        This shows us that <InlineMath math="\text{ELBO}" /> is maximized for a given value of <InlineMath math="\theta" /> when the KL divergence between <InlineMath math="Q(z)" /> and{" "}
        <InlineMath math="p(z | x; \theta)" /> is minimized, which is when they are equal. This is exactly what we saw while deriving the algorithm above.
      </div>
      <Derivation>
        <div className="flex flex-col">
          <BlockMath math="\text{ELBO}(x; Q, \theta) = \mathbb{E}_{z \sim Q} \left[\log \frac{p(x, z; \theta)}{Q(z)} \right]" />
