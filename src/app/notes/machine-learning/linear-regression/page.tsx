...
export default async function LinearRegression() {
    ...
    <div className="from-bgDark to-bgDarkShade flex h-screen w-full justify-center bg-gradient-to-b">
      <div className="flex h-screen w-full flex-row overflow-y-auto">
        ...
        <div className="flex h-full w-9/12 flex-col place-items-center overflow-y-auto py-12 text-opacity-95">
          <div className="flex w-10/12 flex-col rounded-sm bg-white p-12 gap-6">
            <div className="font-medium text-3xl font-serif w-full text-center pb-2">Machine Learning</div>
            ...
            <div className="flex flex-col gap-3 w-full bg-[#CCE5FE] p-10 rounded">
              {data.map((item, index) => (
                <div
                  onClick={() => setTopic(item.title)}
                  key={index}
                  className="cursor-pointer text-[#007bff] hover:text-[#004085] hover:font-normal font-light"
                >
                  {index + 1}. {item.title}
                </div>
              ))}
